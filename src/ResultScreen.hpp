...
// score.dat is a 32-bit wire format. ScoreDat cannot be written directly on
// 64-bit platforms because its ScoreListNode pointer grows from 4 to 8 bytes
// and moves fileLen. Keep the on-disk header architecture-independent.
struct OnDiskScoreDat
{
    u8 xorseed[2];
    u16 csum;
    u16 unk_8;
    u8 unk[2];
    u32 dataOffset;
    u32 scoresPlaceholder;
    u32 fileLen;
};
static_assert(sizeof(OnDiskScoreDat) == 0x14, "score.dat wire header must be 20 bytes");
static_assert(offsetof(OnDiskScoreDat, dataOffset) == 8, "score.dat dataOffset position");
static_assert(offsetof(OnDiskScoreDat, fileLen) == 16, "score.dat fileLen position");
