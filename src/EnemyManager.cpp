    ...
    while (0 <= this->timelineInstr->time)
    {
        if ((ZunBool)(this->timelineTime.current == this->timelineInstr->time))
        {
            ...
            case 10:
            {
                ...
                if (bossIdx < ARRAY_SIZE_SIGNED(this->bosses) && this->bosses[bossIdx] != NULL)
                {
                ...
                else
                {
                    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                                 "[BossProbe] ignored interrupt for invalid boss slot=%u id=%d",
                                 (unsigned)bossIdx, (int)interruptId);
                }
#endif
                break;
            }
