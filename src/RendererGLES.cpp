...
{
#define LOAD(name) p##name = (PFN_##name)SDL_GL_GetProcAddress(#name); if (!p##name) { SDL_LogError(SDL_LOG_CATEGORY_RENDER, "Missing GLES function: %s", #name); return false; }
    LOAD(glCreateShader);
    LOAD(glShaderSource);
...
static GLuint CompileShader(GLenum type, const char *src)
{
    ...
    if (!ok)
    {
        ...
        SDL_LogError(SDL_LOG_CATEGORY_RENDER, "[RendererGLES] Shader compile error: %s", buf);
    }
    return s;
}
...
static GLuint BuildProgram(const char *vsSrc, const char *fsSrc)
{
    ...
    if (!ok)
    {
        ...
        SDL_LogError(SDL_LOG_CATEGORY_RENDER, "[RendererGLES] Program link error: %s", buf);
    }
    pglDeleteShader(vs);
    pglDeleteShader(fs);
    return prog;
...
void RendererGLES::Init(SDL_Window *win, SDL_GLContext ctx, i32 w, i32 h)
{
    ...
    SDL_LogCritical(SDL_LOG_CATEGORY_RENDER,
                    "[RendererGLES] Init logical=%dx%d drawable=%dx%d",
                    w, h, drawW, drawH);

    // Load GL2/GLES2 functions
    if (!LoadGLES2Functions())
    {
        SDL_LogError(SDL_LOG_CATEGORY_RENDER, "[RendererGLES] Failed to load required GL functions");
        return;
    }
    ...
    SDL_LogCritical(SDL_LOG_CATEGORY_RENDER,
                    "[RendererGLES] GL_VERSION: %s", glVer ? glVer : "?");
    SDL_LogCritical(SDL_LOG_CATEGORY_RENDER,
                    "[RendererGLES] GL_RENDERER: %s", glRen ? glRen : "?");

    // Capture SDL/UIKit's drawable target before creating the game's
    // logical-resolution FBO. On iOS, binding framebuffer 0 here would send
    // the final blit to an invisible/default target and leave the screen
    // black even though title/menu JPEG textures decoded successfully.
    GLint drawableFbo = 0;
    GLint drawableRenderbuffer = 0;
    glGetIntegerv(GL_FRAMEBUFFER_BINDING, &drawableFbo);
    glGetIntegerv(GL_RENDERBUFFER_BINDING, &drawableRenderbuffer);
    this->screenFbo = (GLuint)drawableFbo;
    this->screenRenderbuffer = (GLuint)drawableRenderbuffer;
    SDL_LogCritical(SDL_LOG_CATEGORY_RENDER,
                    "[RendererGLES] SDL drawable FBO=%u RBO=%u",
                    (unsigned)this->screenFbo,
                    (unsigned)this->screenRenderbuffer);
    ...
    SDL_LogCritical(SDL_LOG_CATEGORY_RENDER, "[RendererGLES] Init complete");
    ...
    if (drawW != w || drawH != h)
    {
        ...
        {
            pglBindFramebuffer(GL_FRAMEBUFFER, this->screenFbo);
    ...
    pglBindRenderbuffer(GL_RENDERBUFFER, this->screenRenderbuffer);
                                           ...
                                           i32 *outWidth, i32 *outHeight)
{
    ...
    if (this->currentTexture != 0 && this->currentTexture != (u32)-1)
    {
        this->stats.textureBinds++;
...
u32 RendererGLES::LoadSurfaceFromFile(const u8 *data, i32 dataLen, i32 *outWidth, i32 *outHeight)
{
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    // Title, selection, result, and ending JPEGs are 640x480 NPOT textures.
    // GLES 2.0 samples them as black unless both wrap modes are clamped.
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    this->stats.textureBinds++;
    this->stats.textureUploads++;
...
void RendererGLES::CopySurfaceToScreen(u32 surfaceTex, i32 srcX, i32 srcY,
                                        ...
                                        i32 texW, i32 texH)
{
    if (surfaceTex == 0 || texW <= 0 || texH <= 0) return;
    ...
    const u32 previousTexture = this->currentTexture;
    const u8 previousColorOp = this->currentColorOp;

    if (!this->in2DPass) Enter2DPass();
    SetColorOp(0);

    SetTexture(surfaceTex);
    // D3D's surface copy overwrites the framebuffer. It is not a sprite
    // alpha blend, and decoded JPEG alpha bytes must not hide the image.
    ...
    {
        glEnable(GL_BLEND);
        if (this->currentBlendMode == BLEND_MODE_ADD)
            glBlendFunc(GL_SRC_ALPHA, GL_ONE);
        else
            glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    }

    if (previousTexture != (u32)-1)
    ...
    if (previousColorOp != 0xff)
        SetColorOp(previousColorOp);
    else
        this->currentColorOp = 0xff;
}
...
void RendererGLES::DrawScreenSpaceButtons()
{
    ...
    const bool portraitGameplay = IsMobilePortraitGameplayLayout();

    // Recompute presentation layout.
    i32 scaledW, scaledH;
    i32 offsetX, offsetY;
    if (portraitGameplay)
    {
        offsetX = 0;
        offsetY = GetMobilePortraitHeaderHeight(rw, rh);
        scaledW = rw;
        scaledH = rh - offsetY;
    }
    else if (rw * this->screenHeight > rh * this->screenWidth)
    {
        scaledH = rh;
    ...
    // On a 4:3 iPad the game viewport fills the display, so there is no
    // pillarbox to place controls in. Keep the same controls visible just
    // inside the left/right edges in that case.
    const bool hasPillarbox = !portraitGameplay && offsetX >= 5;
    ...
    float portraitButtonScale = (float)rw / 640.0f * 0.82f;
    if (portraitButtonScale < 0.65f)
        portraitButtonScale = 0.65f;
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        float sr = buttons[i].gameRadius * (portraitGameplay ? portraitButtonScale : yScale);

        float sx;
        if (portraitGameplay)
        {
            sx = buttons[i].anchor == ScreenAnchor::RightPillar
                     ? (float)rw - sr - 8.0f
                     : sr + 8.0f;
        }
        else if (buttons[i].anchor == ScreenAnchor::RightPillar)
        {
            ...
            sx = hasPillarbox ? (float)(rw - offsetX) + sr
                              : (float)rw - sr - 8.0f;
            if (sx > (float)rw - sr)
                sx = (float)rw - sr;
        }
    ...
    // --- Text labels using ImGui's default font (parity with Vulkan path) ---
    {
        ...
        if (font && font->IsLoaded() && fontTex != 0)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                float pixelHeight = 16.0f * buttons[i].textScale *
                                    (portraitGameplay ? portraitButtonScale : yScale);
...
void RendererGLES::BlitFBOToScreen()
{
    ...
    // Compute centered 4:3 letterbox/pillarbox. Portrait gameplay is
    // presented separately below after the shader state is prepared.
    i32 scaledW = rw;
    i32 scaledH = rh;
    i32 offsetX = 0;
    i32 offsetY = 0;
    if (!portraitGameplay && rw * this->screenHeight > rh * this->screenWidth)
    {
        scaledH = rh;
        scaledW = rh * this->screenWidth / this->screenHeight;
        offsetX = (rw - scaledW) / 2;
    }
    else if (!portraitGameplay)
    {
        scaledW = rw;
        scaledH = rw * this->screenHeight / this->screenWidth;
        offsetY = (rh - scaledH) / 2;
    ...
    auto drawRegion = [&](i32 srcX, i32 srcY, i32 srcW, i32 srcH,
                          i32 dstX, i32 dstY, i32 dstW, i32 dstH) {
        const f32 left = (f32)dstX / rw;
        const f32 right = (f32)(dstX + dstW) / rw;
        const f32 bottom = 1.0f - (f32)(dstY + dstH) / rh;
        const f32 top = 1.0f - (f32)dstY / rh;
        const f32 u0 = (f32)srcX / this->screenWidth;
        const f32 u1 = (f32)(srcX + srcW) / this->screenWidth;
        const f32 v0 = 1.0f - (f32)(srcY + srcH) / this->screenHeight;
        const f32 v1 = 1.0f - (f32)srcY / this->screenHeight;
        f32 pos[] = { left,bottom,0, right,bottom,0, left,top,0, right,top,0 };
    ...
    if (portraitGameplay)
    {
        const i32 headerHeight = GetMobilePortraitHeaderHeight(rw, rh);
        const i32 leftHudWidth = rw / 2;
        // The original 224x448 right HUD is split into two 224x224 panels
        // so score/lives and power/graze remain readable above the playfield.
        drawRegion(416, 16, 224, 224, 0, 0, leftHudWidth, headerHeight);
        drawRegion(416, 240, 224, 224, leftHudWidth, 0,
                   rw - leftHudWidth, headerHeight);
        // Stretch only the original 384x448 playfield over the remaining
        // portrait area. Touch conversion uses this exact same rectangle.
        drawRegion(32, 16, 384, 448, 0, headerHeight, rw, rh - headerHeight);
    }
    else
    {
        drawRegion(0, 0, this->screenWidth, this->screenHeight,
                   0, 0, rw, rh);
    }
...
{
    static bool loggedFirstSwap = false;
    ...
    if (this->fbo != 0)
    {
        ...
        if (!loggedFirstSwap) { SDL_LogCritical(SDL_LOG_CATEGORY_RENDER, "[RendererGLES] first SDL_GL_SwapWindow (FBO)"); loggedFirstSwap = true; }
#ifndef TH06_MOBILE
    ...
    else
    {
        ...
        this->fogAttribEnabled = false;
        SDL_GL_SwapWindow(this->window);
        if (!loggedFirstSwap) { SDL_LogCritical(SDL_LOG_CATEGORY_RENDER, "[RendererGLES] first SDL_GL_SwapWindow (no FBO)"); loggedFirstSwap = true; }
