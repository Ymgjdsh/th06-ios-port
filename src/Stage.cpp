...
ZunResult Stage::LoadStageData(char *anmpath, char *stdpath)
{
    ...
    if (this->objectsCount < 0 || this->quadCount < 0)
    {
        GameErrorContext::Log(&g_GameErrorContext, TH_ERR_STAGE_DATA_CORRUPTED);
        return ZUN_ERROR;
    }
    ...
    // The STD object table is always an array of 32-bit file offsets.  It is
    // not a native pointer array: treating it as RawStageObject ** corrupts
    // adjacent entries when sizeof(void *) is 8 (iOS arm64 and desktop x64).
    const u32 *objectOffsets = reinterpret_cast<const u32 *>(this->stdData + 1);
    this->objects = static_cast<RawStageObject **>(
        malloc(static_cast<size_t>(this->objectsCount) * sizeof(*this->objects)));
    if (this->objects == NULL && this->objectsCount > 0)
    {
        GameErrorContext::Log(&g_GameErrorContext, TH_ERR_STAGE_DATA_CORRUPTED);
        return ZUN_ERROR;
    }
    ...
    {
        this->objects[idx] = reinterpret_cast<RawStageObject *>(
            reinterpret_cast<u8 *>(this->stdData) + objectOffsets[idx]);
    }
    ...
    for (objIdx = 0; objIdx < this->objectsCount; objIdx++)
    {
        ...
        if (obj->flags & 1 != 0)
        {
            ...
            while (0 <= objQuad->type)
            {
                ...
                objQuad = reinterpret_cast<RawStageQuadBasic *>(
                    reinterpret_cast<u8 *>(&objQuad->type) + objQuad->byteSize);
            }
            if (vmsNotFinished == 0)
            {
    ...
    while (instance->id >= 0)
    {
        ...
        render:
            ...
            while (0 <= curQuad->type)
            {
                ...
                curQuad = reinterpret_cast<RawStageQuadBasic *>(
                    reinterpret_cast<u8 *>(&curQuad->type) + curQuad->byteSize);
            }
            instancesDrawn++;
        }
    skip:
