...
#ifdef TH06_IOS
    // iOS can briefly report a controller candidate while SDL is rebuilding
    // its GameController list. Probing that transient index every game tick
    // produces hundreds of "There are 0 joysticks available" diagnostics.
    // Keep hot-plug support, but limit an unattached-device scan to once every
    // five seconds.
    static Uint64 nextIOSJoystickProbeTick = 0;
    const Uint64 now = SDL_GetTicks64();
    if (now < nextIOSJoystickProbeTick)
    {
        return false;
    }
    nextIOSJoystickProbeTick = now + 5000;
#endif
...
{
#ifdef TH06_IOS
    // iOS builds intentionally omit SDL joystick initialization. Do not ask
    // SDL's game-controller layer to probe an empty device list; that emits
    // one "0 joysticks / mapping" diagnostic per probe on iOS 15.
    return 1;
#endif
    if ((SDL_WasInit(SDL_INIT_JOYSTICK) & SDL_INIT_JOYSTICK) == 0)
    {
