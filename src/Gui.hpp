...
{
    i32 numInstrs;
    // MSG files were authored for the original 32-bit executable. These are
    // file-relative u32 offsets, not native pointers. Treating them as
    // pointers combines adjacent entries on 64-bit builds and corrupts the
    // first boss dialogue address.
    u32 instrOffsets[1];
};
